...
const zendeskSubdomain = "{insert subdomain}";
const zendeskUsername = "{insert email}";
const zendeskApiToken = "{insert token}";
...
const exportApiUrl = `https://${zendeskSubdomain}.zendesk.com/api/v2/incremental/tickets.json?start_time=${start_time}`;
  ...
  request(
    {
      ...
      headers: {
        Authorization:
          "Basic " +
          Buffer.from(`${zendeskUsername}/token:${zendeskApiToken}`).toString(
            "base64"
          )
      },
      json: true
